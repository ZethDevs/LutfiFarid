...
const doodstreamDownloader = require('./features/doodstreamDownloader.js');
...
/**
 * @swagger
 * /api/doodstream-downloader:
 *   get:
 *     tags:
 *       - Downloader
 *     summary: Download video from Doodstream
 *     description: contoh link nya https://dood.li/d/6pg1gc8wqc7h
 *     security:
 *       - ApiKeyAuth: []
 *     parameters:
 *       - in: query
 ...
 *         schema:
 *           type: string
 *         description: The Doodstream URL to download from
 *       - in: query
 *         name: api_key
 *         required: true
 *         schema:
 ...
 *         description: Server error message
 */
app.get('/api/doodstream-downloader', checkApiKey, async (req, res) => {
  const { url } = req.query;
  ...
  try {
    const result = await doodstreamDownloader(url);
    res.json({
      creator: 'ZethDevs',
      status: 'success',
      code: 200,
